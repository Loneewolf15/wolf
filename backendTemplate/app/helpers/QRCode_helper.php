...
class QRCode
{
    ...
    {
        return self::API_URL . $iSize . 'xl' . $iSize . '&cht=qr&chld=' . $sECLevel . '|' . $iMargin . '&chl=' . $this->sData;
    }
    ...
    {
        $qrcode = urldecode($this->_cleanUrl($this->get($iSize)));
       //print_r($qrcode);
        $pattern = "/NICKNAME:(.*)\n/";
        if (preg_match($pattern, $qrcode, $matches)) {
        preg_match("/FN:(.*)/", $qrcode, $matches);
        $fn = $matches[1];
        
        preg_match("/NICKNAME:(.*)/", $qrcode, $matches);
        $nickname = $matches[1];
        
        preg_match("/EMAIL;.*:(.*)/", $qrcode, $matches);
        $email = $matches[1];
